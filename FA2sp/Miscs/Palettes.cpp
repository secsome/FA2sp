...
Palette* Palettes::LoadPalette(ppmfc::CString palName)
{
    ...
    auto nMix = GlobalVars::Dialogs::CLoading->SearchFile(palName);
    if (CMixFile::HasFile(palName, nMix))
    {
        CMixFile::ExtractFile(palName, "TmpPalette.pal", nMix);
        BytePalette buffer;
        HANDLE hFile = CreateFile("TmpPalette.pal", GENERIC_READ, FILE_SHARE_READ, nullptr,
            OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
        if (hFile != INVALID_HANDLE_VALUE)
        {
            DWORD dwReadBytes;
            if (ReadFile(hFile, &buffer, 768, &dwReadBytes, nullptr) && dwReadBytes == 768)
            {
                DeleteFile("TmpPalette.pal");
                auto pPal = GameCreate<Palette>();
                for (int i = 0; i < 256; ++i)
                {
                    pPal->Data[i].R = buffer[i].red << 2;
                    pPal->Data[i].G = buffer[i].green << 2;
                    pPal->Data[i].B = buffer[i].blue << 2;
                }
